...
const serializePost = (p: any, extras?: { commentCount?: number; liked?: boolean }) => ({
  id: p.id.toString(),
...
const postSelect = {
  ...
  barcode: true, shareLocation: true, latitude: true, longitude: true,
  locationHint: true, viewCount: true, likeCount: true, createdAt: true, updatedAt: true,
...
router.get('/', async (req: Request, res: Response) => {
  ...
  res.json({
    posts: posts.map((p) => serializePost(p, { liked: likedSet.has(p.id.toString()) })),
    total, page, limit,
  });
});
...
router.get('/:id', async (req: Request, res: Response) => {
  ...
  let liked = false;
  if (device_uuid) {
    const like = await prisma.postLike.findUnique({ where: { postId_deviceUuid: { postId: id, deviceUuid: device_uuid } } });
    liked = !!like;
  }

  res.json(serializePost(post, { liked }));
});
...
router.post('/', async (req: Request, res: Response) => {
  ...
  const { device_uuid, title, content, price, barcode, share_location, latitude, longitude, location_hint } = parsed.data;
